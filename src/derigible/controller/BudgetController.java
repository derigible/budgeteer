...
class BudgetController extends AbstractController {
    ...
    /**
     * Exclude all transactions of given categories to this budget. This
     * action removes the transaction from the budget list.
     *
    ...
    /**
     * Include all transactions in the given categories. This adds all transactions
     * of the categories to the budget. 
     *
    ...
    /**
     * Exclude all transactions of a given date. These transactions are removed from the budget.
     *
    ...
    public double getBalanceBetweenDatesForAccount(Date start, Date end, String account) {
        return calculate(tlist.filterByAccount(account, tlist.filterByDates(start, end, getValues())));
    }
    ...
    public double getBalanceForCategoryForAccountsBetweenDates(String category, String[] accounts, Date start, Date end) {
        double balance = 0;
        List<Transaction> l0 = tlist.filterByDates(start, end, tlist.filterByCategory(category, getValues()));
        for (String account : accounts) {
            balance += calculate(tlist.filterByAccount(account, l0));
        }
        return balance;
    ...
    public double getBalanceForCategoriesForAccountsBetweenDates(String[] categories, String[] accounts, Date start, Date end) {
        double balance = 0;
        List<Transaction> l0 = tlist.filterByDates(start, end, tlist.filterByCategories(categories, getValues()));
        for (String account : accounts) {
        	balance += calculate(tlist.filterByAccount(account, l0));
        }
        return balance;
