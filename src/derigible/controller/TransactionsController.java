...
public class TransactionsController extends AbstractController {
     ...
     */
    public SubTransaction addSubTransaction(String Guid, double amount)
	    throws IOException {
	...
	try {
	    return this.addSubTransaction((Splittable) this.getTransactions()
		    .getTransactionByGUID(Guid), amount);
	} catch (ClassCastException e) {
	    throw new IOException("Transaction with GUID: " + Guid
		    + " is not of the Splittable type.");
	}
    }
	...
	for (Transaction t : tlist
		.filterByAccount(account, tlist.getExcluded())) {
	    tlist.includeTransaction(t);
	}
    }
	...
	return calculate(tlist.filterByAccount(account,
		tlist.getBetweenDates(start, end)));
    }
	...
	return calculate(tlist.filterByDates(start, end,
		tlist.filterByCategory(category, l)));
    }
	...
	List<Transaction> l = tlist.getByAccount(account);
	return calculate(tlist.filterByDates(start, end,
		tlist.filterByCategories(categories, l)));
    }
	...
	double balance = 0;
	List<Transaction> l = tlist.filterByDates(start, end,
		tlist.getByCategory(category));
	for (String account : accounts) {
	    balance += calculate(tlist.filterByAccount(account, l));
	}
	return balance;
	...
	double balance = 0;
	List<Transaction> l = tlist.filterByDates(start, end,
		tlist.getByCategories(categories));
	for (String account : accounts) {
	    balance += calculate(tlist.filterByAccount(account, l));
	}
	return balance;
     ...
     * @return the income
     */
    public double getIncomeBetweenDatesForAccount(Date start, Date end,
	    String account) {
	return calculateCredits(tlist.filterByAccount(account,
		tlist.filterByDates(start, end, tlist.getCredits())));
    }
	...
	return calculateCredits(tlist.filterByCategory(category,
		tlist.getCredits()));
    }
     ...
     * @return the income
     */
    public double getIncomeForCategoriesBetweenDates(String[] categories,
	    Date start, Date end) {
	return calculateCredits(tlist.filterByDates(start, end,
     ...
     * @return the income
     */
    public double getIncomeForCategoriesForAccount(String[] categories,
	    String account) {
	return calculateCredits(tlist.filterByAccount(account,
		tlist.filterByCategories(categories, tlist.getCredits())));
    }
     ...
     * @return the income
     */
    public double getIncomeForCategoryForAccountBetweenDates(String category,
	    String account, Date start, Date end) {
	return calculateCredits(tlist.filterByAccount(
		account,
		tlist.filterByDates(start, end,
			tlist.filterByCategory(category, tlist.getCredits()))));
    }
     ...
     * @return the income
     */
    public double getIncomeForCategoriesForAccountBetweenDates(
	    String[] categories, String account, Date start, Date end) {
	return calculateCredits(tlist.filterByAccount(account, tlist
		.filterByDates(start, end, tlist.filterByCategories(categories,
			tlist.getCredits()))));
    }
     ...
     * @return the income
     */
    public double getIncomeForCategoryForAccounts(String category,
	    String[] accounts) {
	double balance = 0;
	for (String account : accounts) {
     ...
     * @return the income
     */
    public double getIncomeForCategoriesForAccounts(String[] categories,
	    String[] accounts) {
	double balance = 0;
	for (String account : accounts) {
	    balance += calculateCredits(tlist.filterByAccount(account,
     ...
     * @return the income
     */
    public double getIncomeForCategoryForAccountsBetweenDates(String category,
	    String[] accounts, Date start, Date end) {
	double balance = 0;
	for (String account : accounts) {
	    balance += calculateCredits(tlist.filterByAccount(account, tlist
		    .filterByDates(start, end, tlist.filterByCategory(category,
			    tlist.getCredits()))));
	}
	return balance;
    }
     ...
     * @return the income
     */
    public double getIncomeForCategoriesForAccountsBetweenDates(
	    String[] categories, String[] accounts, Date start, Date end) {
	double balance = 0;
	for (String account : accounts) {
	    balance += calculateCredits(tlist.filterByAccount(
		    account,
		    tlist.filterByDates(
			    start,
			    end,
			    tlist.filterByCategories(categories,
				    tlist.getCredits()))));
	}
	return balance;
    }
	...
	return calculateDebits(tlist
		.filterByAccount(account, tlist.getDebits()));
    }
     ...
     * @return the spending
     */
    public double getSpendingBetweenDatesForAccount(Date start, Date end,
	    String account) {
	return calculateDebits(tlist.filterByAccount(account,
		tlist.filterByDates(start, end, tlist.getDebits())));
    }
	...
	return calculateDebits(tlist.filterByCategory(category,
		tlist.getDebits()));
    }
     ...
     * @return the spending
     */
    public double getSpendingForCategoriesBetweenDates(String[] categories,
	    Date start, Date end) {
	return calculateDebits(tlist.filterByDates(start, end,
     ...
     * @return the spending
     */
    public double getSpendingForCategoryForAccount(String category,
	    String account) {
	return calculateDebits(tlist.filterByAccount(account,
     ...
     * @return the spending
     */
    public double getSpendingForCategoriesForAccount(String[] categories,
	    String account) {
	return calculateDebits(tlist.filterByAccount(account,
		tlist.filterByCategories(categories, tlist.getDebits())));
    }
     ...
     * @return the spending
     */
    public double getSpendingForCategoryForAccountBetweenDates(String category,
	    String account, Date start, Date end) {
	return calculateDebits(tlist.filterByAccount(
		account,
		tlist.filterByDates(start, end,
			tlist.filterByCategory(category, tlist.getDebits()))));
    }
     ...
     * @return the spending
     */
    public double getSpendingForCategoriesForAccountBetweenDates(
	    String[] categories, String account, Date start, Date end) {
	return calculateDebits(tlist
		.filterByAccount(
			account,
     ...
     * @return the spending
     */
    public double getSpendingForCategoryForAccounts(String category,
	    String[] accounts) {
	double debited = 0;
	for (String account : accounts) {
     ...
     * @return the spending
     */
    public double getSpendingForCategoriesForAccounts(String[] categories,
	    String[] accounts) {
	double debited = 0;
	for (String account : accounts) {
	    debited += calculateDebits(tlist.filterByAccount(account,
     ...
     * @return the spending
     */
    public double getSpendingForCategoryForAccountsBetweenDates(
	    String category, String[] accounts, Date start, Date end) {
	double debited = 0;
	for (String account : accounts) {
	    debited += calculateDebits(tlist
		    .filterByAccount(
			    account,
			    tlist.filterByDates(
				    start,
				    end,
				    tlist.filterByCategory(category,
					    tlist.getDebits()))));
	}
	return debited;
    }
     ...
     * @return the spending
     */
    public double getSpendingForCategoriesForAccountsBetweenDates(
	    String[] categories, String[] accounts, Date start, Date end) {
	double debited = 0;
	for (String account : accounts) {
	    debited += calculateDebits(tlist.filterByAccount(
		    account,
		    tlist.filterByDates(
			    start,
			    end,
	...
	for (String account : tlist.getAccounts()) {
		...
		for (int year : tlist.getYearsWithTransactions()) {
		    for (int month : tlist
			    .getMonthsInYearWithTransactions(year)) {
			for (int day : tlist
				.getDaysInMonthInYearWithTransactions(year,
					month)) {
			    GregorianCalendar g = new GregorianCalendar(year,
				    month, day);
			    List<Transaction> l0 = tlist.getByCategoryAndDate(
				    category, g.getTime());
			    List<Transaction> l = tlist.filterByAccount(
				    account, l0);
			    if (l.size() > 1) {
				...
				for (Map.Entry<Double, LinkedList<Transaction>> entry : amounts
					...
					for (Transaction t : entry.getValue()) {
						...
						LinkedList<Transaction> templ = new LinkedList<Transaction>();
						templ.add(t);
						desc.put(t.getDescription()
							.toLowerCase(), templ);
					    }
					}
					for (Map.Entry<String, List<Transaction>> descEntry : desc
						.entrySet()) {
					    if (descEntry.getValue().size() > 1) {
						Transaction[] temparray = new Transaction[descEntry
							.getValue().size()];
						for (int i = 0; i < descEntry
							.getValue().size(); i++) {
						    temparray[i] = descEntry
							    .getValue().get(i);
						}
						dupes.add(temparray);
					    }
     ...
     * @throws IOException
     */
    protected BudgetController createBudgetFromFile(String budgetName, File file)
	    throws IOException {
	return new BudgetController(tlist, budgetName, new CSVToBudget(file,
		tlist).data_to_transactions().getTransactions());
    }
