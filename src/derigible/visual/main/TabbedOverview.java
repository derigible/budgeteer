...
public class TabbedOverview {
	...
	protected void applyFilter(Filter filter){
		...
		switch(filter){
			...
			case ACCOUNTS:
				...
				applyFilter(tc.getTransactions().getByAccounts(acts),
						bal);
				break;
			case DATE:
				gc1 = new GregorianCalendar(leftBar.getYear1(), leftBar.getMonth1() - 1, leftBar.getDay1());
				applyFilter(tc.getTransactions().getByDate(gc1.getTime()), tc.getBalanceBetweenDates(gc1.getTime(), gc1.getTime()));
				break;
			case DATES:
				gc1 = new GregorianCalendar(leftBar.getYear1(), leftBar.getMonth1() - 1, leftBar.getDay1());
				gc2 = new GregorianCalendar(leftBar.getYear2(), leftBar.getMonth2() - 1, leftBar.getDay2());
				applyFilter(tc.getTransactions().getByDate(gc1.getTime()), tc.getBalanceBetweenDates(gc1.getTime(), gc2.getTime()));
				break;
			case CATEGORIES_AND_DATE:
				cats = leftBar.getCategoriesList().getSelection();
				gc1 = new GregorianCalendar(leftBar.getYear1(), leftBar.getMonth1() - 1, leftBar.getDay1());
				applyFilter(tc.getTransactions().getByCategoriesAndDate(cats, gc1.getTime()), 
						tc.getBalanceForCategoriesBetweenDates(cats, gc1.getTime(), gc1.getTime()));
				break;
			case CATEGORIES_AND_DATES:
				...
				gc2 = new GregorianCalendar(leftBar.getYear2(), leftBar.getMonth2() - 1, leftBar.getDay2());
				applyFilter(tc.getTransactions().getByCategoriesAndDate(cats, gc1.getTime()), 
						tc.getBalanceForCategoriesBetweenDates(cats, gc1.getTime(), gc2.getTime()));
				break;
			case ACCOUNTS_AND_DATE:
				acts = leftBar.getListAccounts().getSelection();
				gc1 = new GregorianCalendar(leftBar.getYear1(), leftBar.getMonth1() - 1, leftBar.getDay1());
				for(String act : acts){
					bal += tc.getBalanceBetweenDatesForAccount(gc1.getTime(), gc1.getTime(), act);
				}
				applyFilter(tc.getTransactions().filterByDate(gc1.getTime(), tc.getTransactions().getByAccounts(acts)), bal);
				break;
			case ACCOUNTS_AND_DATES:
				...
				gc2 = new GregorianCalendar(leftBar.getYear2(), leftBar.getMonth2() - 1, leftBar.getDay2());
				for(String act : acts){
					bal += tc.getBalanceBetweenDatesForAccount(gc1.getTime(), gc2.getTime(), act);
				}
				applyFilter(tc.getTransactions().filterByDates(gc1.getTime(), gc2.getTime(), 
			...
			case CATEGORIES_AND_ACCOUNTS_AND_DATE:
				...
				applyFilter(tc.getTransactions().filterByAccounts(acts, 
						tc.getTransactions().getByCategoriesAndDate(cats, gc1.getTime())),
						bal + tc.getBalanceForCategoriesBetweenDates(cats, gc1.getTime(), gc1.getTime()));
				break;
			case CATEGORIES_AND_ACCOUNTS_AND_DATES:
				...
				applyFilter(tc.getTransactions().filterByAccounts(acts, 
						tc.getTransactions().getByCategoriesAndDates(cats, gc1.getTime(), gc2.getTime())),
						bal + tc.getBalanceForCategoriesBetweenDates(cats, gc1.getTime(), gc2.getTime()));
				break;
		}
	}
	...
	private void showFilteredValues(java.util.List<Transaction> trans){
		...
		Table newtable = new TransactionsTable(tableTabs, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI).getTable();
		tables.add(newtable);
	...
	private void setTableValues(Transactions trans, Table table0){
		table0.removeAll();
		...
		mntmOpen.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try {
					...
					setTableValues(tc.getTransactions(), table);
