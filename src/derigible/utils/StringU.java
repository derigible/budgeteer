...
public final class StringU {
	...
	/**
	 * Converts a string to a beautified output. This basically cleans out extra
	 * whitespace from the string, and converts it to proper case. If the name
	 * is already mixed case, it's assumed that is on purpose, so this method
	 * will not convert the capitalization, but still clean up extra whitespace.
	 * 
	 * Cases: - if the input is null, return null - if the input is a 0-length
	 * string, return an empty string - if the input is mixed case (contains
	 * upper and lower case), trim out extra spaces - if the input is all upper
	 * case or all lower case, capitalize first letter of each word, lower case
	 * other letters, and trim out extra spaces
	 * 
	 * Method is quite fast - use it nearly everywhere a name string is output
	...
	public static String formatNameString(String input) {
		if (input == null) {
			...
			String[] names = input.split(" "); // Split on the spaces in between
			// to test each word for casing
			// individually
			...
			for (int i = 0; i < chars.length; i++) {
				...
				// If character was uppercase, do nothing (will return 0 if all
				// are uppercase)
			}
			input = "";
	...
	/**
	 * Does pretty much the same thing as formatNameString with the exception
	 * that it treats all strings, mixed case or not, as poorly formatted and
	 * beautifies them.
	 * 
	 * Example output:
	 * 
	 * [null] =&gt; [null] [] =&gt; [] [ ] =&gt; [] [A] =&gt; [A] [b] =&gt; [B]
	 * [bob] =&gt; [Bob] [BOB] =&gt; [Bob] [McDonald] =&gt; [Mcdonald] [a b]
	 * =&gt; [A B] [A B] =&gt; [A B] [jim jones] =&gt; [Jim Jones] [JOHN JOE
	 * DOE] =&gt; [John Joe Doe] [Mike McRae] =&gt; [Mike Mcrae] [mike McRae]
	 * =&gt; [Mike Mcrae] [ jim jones] =&gt; [Jim Jones] [JOHN JOE DOE] =&gt;
	 * [John Joe Doe] [Mike McRae ] =&gt; [Mike Mcrae] [ mike McRae ] =&gt;
	 * [Mike Mcrae]
	 * 
	 * 
	 * @param input
	...
	/**
	 * Takes a string input and cleans up the whitespace and returns a lower
	 * case version of the string. Returns null if string is empty.
	 * 
	 * @param input
